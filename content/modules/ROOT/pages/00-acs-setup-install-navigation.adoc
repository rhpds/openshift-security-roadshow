...
NOTE: Pull *Debian* `python:3.12-slim-bookworm`, not Alpine and not `node:*`. The patient-portal frontend is Python (`main.py` / pip), so a Node base cannot build it. Alpine images make Quay/Clair report Passed or Unknown-only (no severity). Debian's tracker gives Clair real Low/Medium/High/Critical on this image.
...
[source,bash,subs="+macros,+attributes"]
----
[demo-user@bastion ~]$ podman pull python:3.12-slim-bookworm
...
====
This command tags the Debian Python image with your Quay repository and username, allowing you to accurately push the image in a later step.
====

[source,sh,subs="attributes",role=execute]
----
podman tag docker.io/library/python:3.12-slim-bookworm $QUAY_URL/$QUAY_USER/python-alpine-golden:0.1
----
...
docker.io/library/python                                                                    3.12-slim-bookworm  451fdb03e172  5 weeks ago  50.6 MB
----
...
[start=3]
. Point both `FROM` lines at your golden image, and switch the Alpine `apk`/`adduser` lines to Debian `apt`/`useradd` (the stock demo-apps Dockerfile is still Alpine):
...
sed -i 's|apk add --no-cache bash|apt-get update \&\& apt-get install -y --no-install-recommends bash \&\& rm -rf /var/lib/apt/lists/*|' $TUTORIAL_HOME/app-images/frontend/Dockerfile
sed -i 's|adduser -S fritz -G root|useradd -r -g root -d /home/fritz -m fritz|' $TUTORIAL_HOME/app-images/frontend/Dockerfile
----
...
This tab lists the images and tags that have been uploaded, including size and security-scan status. You will scan this image with RHACS in the next section.
...
(TOTAL-COMPONENTS: 60, TOTAL-VULNERABILITIES: 171, LOW: 106, MODERATE: 34, IMPORTANT: 23, CRITICAL: 8)

+---------------+----------------------+------------------+-----------+------+
|   COMPONENT   |       VERSION        |       CVE        | SEVERITY  | CVSS |
+---------------+----------------------+------------------+-----------+------+
|    django     |        1.11.29       |  CVE-2025-64459  | CRITICAL  | 9.1  |
| libsqlite3-0  |    3.40.1-2+deb12u2  |   CVE-2025-7458  | CRITICAL  | 9.1  |
|   perl-base   |    5.36.0-7+deb12u3  |   CVE-2026-8376  | CRITICAL  | 9.8  |
|    pyyaml     |         3.12         |  CVE-2020-14343  | CRITICAL  | 9.8  |
|    zlib1g     |   1:1.2.13.dfsg-1    |  CVE-2023-45853  | CRITICAL  | 9.8  |
|     flask     |          0.5         |  CVE-2023-30861  | IMPORTANT | 7.5  |
+---------------+----------------------+------------------+-----------+------+
WARN:   A total of 171 unique vulnerabilities were found in 60 components
----

NOTE: Counts change as vulnerability feeds update. You should see **Critical** findings on the packages the Dockerfile installed for the lab (`Django==1.11.29`, `PyYAML==3.12`) plus Debian Bookworm libraries from the golden base (`perl-base`, `zlib1g`, `libsqlite3-0`). `Flask==0.5` shows as **Important**. That is the scanner later modules use for policy and reporting.
